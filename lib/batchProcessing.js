...
export async function processBatchPreview(batchId, config) {
  try {
    ...
    // Find all files belonging to this batch in memory first
    ...
    if (batchFiles.length === 0) {
      try {
        ...
        if (persistentResult.success && persistentResult.files.length > 0) {
          // Load the first file from persistent storage for preview
          const { loadBatchFile } = await import('./fileStorage.js');
          const firstFileResult = await loadBatchFile(batchId, persistentResult.files[0].fileId);
          
          if (firstFileResult.success) {
            batchFiles.push({
              fileId: persistentResult.files[0].fileId,
              ...firstFileResult.file
            });
          }
        }
      } catch (error) {
